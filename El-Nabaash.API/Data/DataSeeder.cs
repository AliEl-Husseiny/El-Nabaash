...
namespace El_Nabaash.API.Data
{
    public class DataSeed
    {
        ...
        {
            await using var dbContextSvc = svcProvider.GetRequiredService<AppDbContext>();
        ...
        private static async Task SeedSitesAsync(AppDbContext context)
        {
        ...
        // Seed base artifacts
        private static async Task SeedArtifactsAsync(AppDbContext context)
        {
        ...
        {
            var dbContext = svcProvider.GetRequiredService<AppDbContext>();
        ...
        public static async Task SeedArtifactMediaFilesAsync(AppDbContext context)
        {
        ...
        private static async Task ResetPostgresSequencesAsync(AppDbContext context)
        {
     ...
     SELECT setval(
        ...
        private static async Task SeedUsersAsync(IServiceProvider svcProvider)
        {
            ...
            // Admin
            var admin = await GetOrCreateUserAsync(userManager, "Admin", "User", "admin@aeon.org", "Admin123!",
                "Admin");
            if (admin != null)
            {
            ...
            // Archivist
            var archivist = await GetOrCreateUserAsync(userManager, "Archivist", "User", "archivist@aeon.org",
                "Archivist123!", "Archivist");
            if (archivist != null)
            {
            ...
            // Researcher
            var researcher = await GetOrCreateUserAsync(userManager, "Researcher", "User", "researcher@aeon.org",
                "Researcher123!", "Researcher");
            if (researcher != null)
            {
        ...
        private static async Task AddClaimIfNotExists(UserManager<ApplicationUser> userManager, ApplicationUser user,
